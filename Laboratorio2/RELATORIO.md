...
qualidade, e a maior parte do que se ouve é relato anedótico. Este laboratório
...
- **Assistente de IA: Claude Sonnet 5, fixo em todos os 6 trials com
  ...
  "com IA" seja comparável dentro do próprio experimento, pois trocar de
  ferramenta entre trials introduziria uma variável de confusão.

- **Linguagem: Python, métricas via Radon.** A especificação
...
- **Pareamento estatístico por kata, não por integrante.** Consequência
  ...
  o par (manual, com IA) de cada um dos 6 katas, e não o par de trials de uma
...
- **SciPy (`scipy.stats.wilcoxon`):** teste de Wilcoxon para amostras
  ...
  aprofundamento.
- **Claude Sonnet 5:** assistente de IA usado em todos os 6 trials com IA.
...
fornece LOC total, branco, comentário e NCLOC agregados, mas não diz **onde**,
...
idênticas), um refinamento que o LOC total sozinho (já exigido pelo
...
Mediana de **798,5 s (13min18s) manual vs. 65,0 s (1min05s) com IA**, uma redução
...
experimento e é coerente com o esperado, já que resolver um kata de
...
ou sem IA, então não há variação nos dados para testar. Isso não significa que a
...
de complexidade mais concentrada por linha, o que é plausível, já que o código da
...
integrantes, não apenas o uso de IA. Essa limitação foi assumida em troca de uma
...
índice de manutenibilidade maior, sem alterar de forma
...
ambos os tratamentos chegassem a 100% de sucesso. É um resultado tão válido
...
Vale destacar, no entanto, o contexto dessa vantagem: os katas são
exercícios públicos, com enunciado bem definido e sem ambiguidade de
requisito. É o tipo de problema em que a IA tende a reconhecer um padrão já
visto no treinamento, o que ajuda a explicar a diferença de ~12×. Isso é
coerente com Kwa et al. (2025), que mostram que a taxa de sucesso de agentes
de IA cai conforme cresce o tempo que um humano levaria para concluir a
tarefa. Em um projeto com regras de negócio, decisões de arquitetura e
requisitos ambíguos, a codificação manual teria mais chance de competir,
pois boa parte do trabalho passaria a ser entender o problema antes de
resolvê-lo. Orlanski et al. (2026) reforçam esse ponto: em especificações
que evoluem e exigem decisões de arquitetura, nenhum dos 15 agentes avaliados
resolveu um problema de ponta a ponta, e o código gerado ficou mais verboso
e estruturalmente mais erodido que o de repositórios humanos. Em escala
organizacional, o relatório DORA (2024) aponta que a IA aumenta a
produtividade individual, mas reduz a estabilidade e a vazão da entrega de
software. A conclusão deste laboratório deve, portanto, ser lida com esse
recorte: a IA venceu com folga no tipo de tarefa testado, não
necessariamente em qualquer tarefa de programação.
...
- KWA, Thomas et al. Measuring AI Ability to Complete Long Software Tasks.
  arXiv:2503.14499, METR, 2025. Disponível em:
  [https://arxiv.org/abs/2503.14499](https://arxiv.org/abs/2503.14499).
- ORLANSKI, Gabriel et al. SlopCodeBench: Benchmarking How Coding Agents
  Degrade Over Long-Horizon Iterative Tasks. arXiv:2603.24755, 2026.
  Disponível em: [https://arxiv.org/abs/2603.24755](https://arxiv.org/abs/2603.24755).
- DORA (Google Cloud). Accelerate State of DevOps Report 2024. 2024.
  Disponível em: [https://dora.dev/research/2024/dora-report/](https://dora.dev/research/2024/dora-report/).